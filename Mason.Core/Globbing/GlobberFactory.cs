...
namespace Mason.Core.Globbing
{
	...
	internal class GlobberFactory
	{
		...
		public Globber? Create(string path)
		{
			...
			var globbers = new List<Globber>(count);

			var countOneOff = count - 1;
			for (var i = 0; i < countOneOff; ++i)
			{
				var globber = FromSegment(split[i]);
			...
			{
				var globber = split[countOneOff] switch
				{
					"" => StatelessGlobbers.Current,
					var name => FromSegment(name)
				};
